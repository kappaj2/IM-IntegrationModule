package za.co.ajk.integration.service.messaging.googlepubsub.impl;
...
import za.co.ajk.integration.config.MessageImplementationCondition;
import za.co.ajk.integration.config.PubSubMessagingProperties;
import za.co.ajk.integration.enums.PubSubMessageType;
import za.co.ajk.integration.service.messaging.IMMessageProcessor;
import za.co.ajk.integration.service.messaging.InterModulePubSubMessage;
import za.co.ajk.integration.service.messaging.googlepubsub.GoogleChannelManager;
...
public class GoogleChannelManagerImpl implements GoogleChannelManager {
    ...
    public PubSubInboundChannelAdapter messageChannelAdapterGenericSub(
        ...
        try {
            PubSubInboundChannelAdapter adapter =
                new PubSubInboundChannelAdapter(pubSubTemplate, "IntegrationGenericSub");
            adapter
                .setOutputChannel(
    ...
    @Bean
    public PubSubInboundChannelAdapter messageChannelAdapterIntegrationSub(
        @Qualifier("pubsubInputChannel")
            MessageChannel inputChannel) {
        try {
            PubSubInboundChannelAdapter adapter =
                new PubSubInboundChannelAdapter(pubSubTemplate, "IntegrationTopicSub");
            adapter
                .setOutputChannel(
                    inputChannel); // looks like the channel to ack on (thus the input channel - confusing!)
            adapter.setAckMode(AckMode.MANUAL);
